...
arch-chroot /mnt pacman -Q limine efibootmgr btrfs-progs cryptsetup sd-encrypt
...
cp /usr/share/limine/limine_X64.EFI esp/EFI/limine

find /boot -iname "*.efi"

efibootmgr --create --disk $SYSTEM_DISK --part 1 \
    --label "Arch Linux Limine Boot Loader" --loader 'esp/EFI/limine/limine_x64.efi' --unicode
efibootmgr -v
...
cat > /EFI/limine/limine.conf << ENTRYEOF
timeout: 1
/Arch Linux
    protocol: linux
